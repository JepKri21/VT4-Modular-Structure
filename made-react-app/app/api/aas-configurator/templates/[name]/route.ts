...
function loadFieldOptions(templateName: string, optionsFile: string): { options: OptionsMap; derived: DerivedMap; operations: OperationsMap } {
  try {
...
export async function GET(
  ...
  for (const candidate of candidates) {
    ...
    if (fs.existsSync(filePath)) {
      ...
      const { options, derived, operations } = loadFieldOptions(name, OPTIONS_FILE);
